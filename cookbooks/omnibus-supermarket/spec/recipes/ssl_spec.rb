describe 'omnibus-supermarket::ssl' do
  let(:chef_run) do
    ChefSpec::Runner.new do |node|
      node.set['supermarket']['ssl']['openssl_bin'] = '/usr/bin/true'
  ...
  context 'when an ssl certificate is not defined' do
    it 'creates /var/opt/supermarket/ssl/ca/chefspec.local.key' do
      expect(chef_run).to create_file_if_missing(
        '/var/opt/supermarket/ssl/ca/chefspec.local.key'
      ).with(
        owner: 'root',
        group: 'root',
        mode: '0640',
    ...
    it 'creates /var/opt/supermarket/ssl/ca/chefspec.local-ssl.conf' do
      expect(chef_run).to create_template_if_missing(
        '/var/opt/supermarket/ssl/ca/chefspec.local-ssl.conf'
      ).with(
