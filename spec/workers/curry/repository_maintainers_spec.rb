...
RSpec.describe Curry::RepositoryMaintainersWorker, :type => :worker do
  ...
  let!(:sub_components) do
    { "Core" => {
        "lieutenant" => "janedoe",
        "maintainers" => "fred",
        "Sub" => {"text" => "foo", "maintainers" => ["xara", "johndoe"] }
      }
    }
  end
  ...
  describe "#resolve" do
    context "with an array" do
      it "should return the correct users" do
        expect(worker.resolve(["janedoe","xara"])).to eql([jane, xara])
      end

      it "should filter unexpected users" do
        expect(worker.resolve(["janedoe","janetdoe", "xara"])).to eql([jane, nil, xara])
      end
    end
